...
object Dijkstra {
  ...
  private def initializeVerticesMap(start: String, stops: Seq[Stop]) = {
    var verticesFromStart: SortedMap[String, Vertex] = stops
      .map(stop => stop.id -> Vertex.init(stop))
      .to(SortedMap)
  ...
  private def dijkstraRecursive(
      currentId: String,
      edges: Seq[Edge],
      vertices: SortedMap[String, Vertex]
  ): Unit = {
    ...
    val neighboursMap = vertices.filter { case (id, _) => neighboursIds.contains(id) }

        //println(s"Dijkstra - ${currentVertex.id}")
        //println(s"- neighbours: ${neighboursMap.keySet}")
    

    // 3. For each neighbour
    neighboursMap.foreach(neighbour =>
      updateNeighbourIfNeeded(currentVertex, neighbour._1, vertices)
    )
    ...
    val nonVisitedNeighbours = neighboursMap.values.toSeq
        .filter(v => !v.visited)
        .sortBy(_.distanceFromInitial)(Ordering.Double.TotalOrdering)
        
    nonVisitedNeighbours.foreach(neighbour => dijkstraRecursive(neighbour.id, edges, vertices))
    
  }

  private def updateNeighbourIfNeeded(
      currentVertex: Vertex,
      neighbourId: String,
      vertices: SortedMap[String, Vertex]
  ) = {
    var neighbour = vertices(neighbourId)
        if("XBON" == currentVertex.id) {
    //println(s"- Updating neighbour ${neighbour.id} ")
    //println(s"---- Distance ${neighbour.distanceFromInitial}")
    //println(s"---- Path ${neighbour.pathFromInitial}")
    }
  ...
  def printRouteFromStartToEnd(
      end: String,
      verticesFromStart: SortedMap[String, Vertex]
  ) = {
...
case class Vertex(
    id: String,
    distanceFromInitial: Double,
    pathFromInitial: Seq[String],
    visited: Boolean,
    stop: Stop
) {
...
object Vertex {
  def init(stop: Stop) =
    Vertex(stop.id, Double.PositiveInfinity, Seq.empty, false, stop)
}
