...
def set_current_node(visited, currentDistances):
    # Use min() to find the node with the smallest distance
    # Only consider nodes that have NOT been visited yet
    nonVisited = (node for node in currentDistances if node not in visited) # generator of unvisited nodes
    return min(
        nonVisited,  
        key=lambda n: currentDistances[n]  # compare nodes by their distance values
    )
